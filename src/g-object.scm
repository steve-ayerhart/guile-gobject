(define-module (g-object)
  ...
  #:use-module (g-object g-parameter)
  #:use-module (g-object g-signal)
...
(eval-when (expand load eval)
  (load-extension (extensions-library "gobject") "scm_gobject_gobject_init"))
...
(define-method (compute-get-n-set (class <g-object-class>) slotdef)
  (let ((name (slot-definition-name slotdef)))
    (case (slot-definition-allocation slotdef)
      ((#:g-property) (list (λ (o) (get-property o name))
            ...
            (if (eq? (car args) #:g-signal)
                (let ((signal (cadr args)))
                  (if (not (and (list? signal) (>= (length signal) 2)))
                      (error "Invalid signal specification: ~a" signal))
                  (let* ((name (car signal))
...
@lisp
 ...
 (define-class <test> (<gobject>)
  ...
  (pub-data #:g-param (list <gparam-long> #:name 'test))
...
(define-class/docs <g-param-object> (<g-param>)
  ...
  (object-type
   ...
   #:pred (λ (x) (is-a? x <g-object-class>)))
  #:value-type <g-object>
...
(define-method (set-property! (object <g-object>) (name <symbol>) value)
  "The default implementation of @code{set-property!}, which sets
slots on the object."
  (if (class-slot-definition (class-of object) name)
      (slot-set! object name value)
      (error "Properties added after object definition must be accessed via custom property methods: ~a" name)))
...
(define-method (get-property (object <g-object>) (name <symbol>))
