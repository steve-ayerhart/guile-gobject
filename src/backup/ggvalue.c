...
scm_gvalue_struct_free (SCM object)
{
  ...
  if (value)
    {
      g_debug ("freeing GValue %p", value);
      g_value_unset (value);
  ...
  if (class != scm_class_gvalue)
    {
      ...
      g_debug ("Bound SCM %p to GValue %p (%s)", instance, value, g_type_name (type));
    }
  else
    {
      g_debug ("Bound SCM %p to generic GValue %p", instance, value);
    }
...
#define FUNC_NAME "gvalue-ref"
{
  ...
  switch (fundamental)
    ...
    default:
      {
        g_debug ("fall through for gvalue");
...
#define FUNC_NAME "gvalue-set!"
{
  ...
  if (SCM_GVALUEP (value))
    ...
    if (g_type_is_a (scm_c_gtype_class_to_gtype (scm_class_of (value)),
      ...
      {
      scm_misc_error (FUNC_NAME, "Can't make ~a into ~a",
...
{
  g_debug (" copying gboxed %p", boxed);
...
{
  g_debug (" freeing gboxed %p", boxed);
...
{
  g_debug ("scm_c_gvalue_new_from_boxed");
...
void
ggobject_gvalue_init (void)
{
  g_debug ("ggobject_gvalue_init");
#ifndef SCM_MAGIC_SNARFER
#include "ggvalue.x"
#endif
  scm_allocate_instance = scm_permanent_object (scm_variable_ref (scm_c_public_lookup ("oop goops", "allocate-instance")));
  ...
  scm_class_gvalue =
    scm_permanent_object (scm_variable_ref (scm_c_lookup ("<gvalue>")));

  g_debug ("ggobject_gvalue_end");
}
