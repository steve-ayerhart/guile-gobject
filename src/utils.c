...
char *
scm_c_gtype_name_to_scheme_name (const char *gtype_name)
{
  size_t len = strlen (gtype_name);
  GString *name = g_string_new (NULL);
  gboolean was_lower = FALSE;

  for (size_t ch = 0; ch < len; ch++)
    {
      if (g_ascii_islower (gtype_name[ch]))
        {
          g_string_append_c (name, gtype_name[ch]);
          was_lower = TRUE;
        }
      else if (gtype_name[ch] == '_' || gtype_name[ch] == '-')
        {
          g_string_append_c (name, '-');
          was_lower = FALSE;
        }
      else if (g_ascii_isdigit (gtype_name[ch]))
        {
          g_string_append_c (name, gtype_name[ch]);
          was_lower = FALSE;
        }
      else if (g_ascii_isupper (gtype_name[ch]))
        {
          if (was_lower)
            {
              g_string_append_c (name, '-');
          ...
          g_string_append_c (name, g_ascii_tolower (gtype_name[ch]));
          was_lower = FALSE;
        }
    }
